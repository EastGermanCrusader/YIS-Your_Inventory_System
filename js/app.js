...
(function () {
  ...
  function isQrDeepLink() {
    return new URLSearchParams(window.location.search).has('sn');
  }

  function applyAkteViewMode(mobileSecured) {
    var modal = $('akte-modal');
    document.body.classList.toggle('akte-mobile-secured', mobileSecured);
    modal.classList.toggle('akte-mobile-secured', mobileSecured);
    var downloadBtn = $('btn-download-qr');
    var uploadHint = modal.querySelector('.akte-upload-hint');
    if (downloadBtn) downloadBtn.hidden = mobileSecured;
    if (uploadHint) uploadHint.hidden = mobileSecured;
  }
  ...
  function renderPublicPlaceholder() {
    ...
    $('public-sn-open').addEventListener('click', function () {
      ...
      if (sn) openAkte(sn, false, { mobileSecured: isMobileDevice() });
  ...
  async function openAkte(serial, showEditActions, options) {
    options = options || {};
    var mobileSecured = !!options.mobileSecured;
    var canEdit = showEditActions && !mobileSecured;
    ...
    $('akte-actions').hidden = !canEdit;
    await renderQr($('akte-qr'), it.seriennummer);
    renderAkteDateien(currentDateien, canEdit);
    applyAkteViewMode(mobileSecured);
  ...
  async function boot() {
    try {
      ...
      if (sn) {
        await openAkte(sn, isLoggedIn(), {
          mobileSecured: isQrDeepLink() && isMobileDevice()
        });
      }
    } catch (err) {
